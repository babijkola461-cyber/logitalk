...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.label_theme = CTkOptionMenu(self.frame, values=["Темна","Сиітла"], command=self.change_theme)
        ...
        self.messege_input = CTkEntry(self, placeholder_text="")
    ...
    def adaptive_ui(self):
        self.chat_text.configure(
            width=self.winfo_height() - self.frame.winfo_width(),
            height=self.winfo_height() - self.messege_input.winfo_height() - 30
        )
        self.chat_text.place(x=self.frame.winfo_width() - 1)

        self.messege_input.configure(
            width=self.winfo_width() - self.frame.winfo_width() - self.send_button.winfo_width()
        )
        self.messege_input.place(
